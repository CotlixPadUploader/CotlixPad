...
namespace ColtixPad.Pages
{
    public class Player : MonoBehaviour
    {
        ...
        public void InitializePage()
        {
            ...
            GameObject cameraObject = new GameObject("ColtixPad_SpectateCamera");
            ...
            {
                GorillaPlayerScoreboardLine scoreboardLine = GorillaScoreboardTotalUpdater.allScoreboardLines.FirstOrDefault(line => line.linePlayer == targetRig.Creator);
        ...
        private VRRig _lastNotifiedRig; // tracks who we last sent a cheat notification for

        // Called externally (e.g. from scoreboard X button) to force-target a rig
        public static Player Instance { get; private set; }
        public void Awake() => Instance = this;
        public static void RequestTarget(VRRig rig)
        {
            if (Instance == null) return;
            Instance.targetRig = rig;
...
{targetRig.fps}
{GetPing(targetRig)}
{GetCreationDate(targetRig.Creator.UserId, (str) => updateTime = 0f)}");
                    ...
                    foreach (DictionaryEntry dictionaryEntry in targetRig.Creator.GetPlayerRef().CustomProperties)
                        customProps[dictionaryEntry.Key.ToString().ToLower()] = dictionaryEntry.Value;

                    foreach (var mod in modDictionary.Where(mod => customProps.ContainsKey(mod.Key.ToLower()) && MatchesMod(mod.Value, customProps[mod.Key.ToLower()])))
                    {
                        if (mod.Value.legal)
                    ...
                    if (usage.activeSelf)
                    {
                        ...
                        if (illegalMods.Count > 0)
                            modLabel = $"<color=grey>[</color><color=red>Cheater</color><color=grey>]</color> Mods (<color=green>{legalMods.Count}</color>:<color=red>{illegalMods.Count}</color>)";
                        else if (legalMods.Count > 0)
                            modLabel = $"<color=grey>[</color><color=green>Clean</color><color=grey>]</color> Mods (<color=green>{legalMods.Count}</color>:<color=red>0</color>)";
                        else
                            modLabel = "No mods detected";
                        usage.transform.Find("Mods/Text").GetComponent<TextMeshPro>().SafeSetText(modLabel);

                        // Fire a DTASLOI-style notification the first time we detect cheats on this player
                        if (illegalMods.Count > 0 && targetRig != _lastNotifiedRig)
                        {
                            _lastNotifiedRig = targetRig;
                            string cheatList = string.Join(", ", illegalMods);
                            Notifications.SendNotification(
                                $"<color=grey>[</color><color=red>Cheater</color><color=grey>]</color> <color=white>{targetRig.playerNameVisible}</color> has the following cheats: <color=red>{cheatList}</color>",
                                8000);
                        }
                        else if (illegalMods.Count == 0 && targetRig != _lastNotifiedRig)
                        {
                            _lastNotifiedRig = targetRig;
                        }
                    }
        ...
        {
            public string name; public bool legal; public string value; // optional: if set, prop value must match
        }

        private static bool MatchesMod(ModInfo mod, object propValue)
        {
            if (string.IsNullOrEmpty(mod.value)) return true; // key-only check
            return propValue != null && propValue.ToString().Equals(mod.value, System.StringComparison.OrdinalIgnoreCase);
        }

        public static readonly Dictionary<string, ModInfo> modDictionary = new Dictionary<string, ModInfo> {
            // ── Previously known ──────────────────────────────────────────────
            { "infoWatchMenuName", new ModInfo { name = "Seralyth Menu", legal = false, value = "Seralyth Menu" } },
            { "CustomMenuName",    new ModInfo { name = "Seralyth Menu", legal = false, value = "Seralyth Menu" } },
            { "customMenuName",    new ModInfo { name = "Seralyth Menu", legal = false, value = "Seralyth Menu" } },
            ...
            { "DTAOI", new ModInfo { name = "DTAOI (has Console)", legal = false } },
            { "DTASLOI", new ModInfo { name = "DTASLOI (has Console)", legal = false } },
            ...
            // ── New detections extracted from DangThatsAShitLoadOfInfo ────────
            { "stupid", new ModInfo { name = "Stupid Menu", legal = false } },
            { "symex", new ModInfo { name = "Symex", legal = false } },
            { "ccm", new ModInfo { name = "ColossalCheatMenu", legal = false } },
            { "colossal", new ModInfo { name = "ColossalCheatMenu", legal = false } },
            { "sodium", new ModInfo { name = "Sodium", legal = false } },
            { "console", new ModInfo { name = "iiDk Console", legal = false } },
            { "goldentrophy_Console", new ModInfo { name = "iiDk Console (Admin)", legal = false } },
            { "Euphoric", new ModInfo { name = "Euphoric", legal = false } },
            { "iimenu", new ModInfo { name = "iiMenu", legal = false } },
            { "resurgence", new ModInfo { name = "Resurgence", legal = false } },
            { "rblxcheezburger", new ModInfo { name = "RblxCheezeburger", legal = false } },
            { "mmmchezburger", new ModInfo { name = "MmmChezburger", legal = false } },
            { "rbsword", new ModInfo { name = "RbSword", legal = false } },
            { "bad boy", new ModInfo { name = "Bad Boy Menu", legal = false } },
            { "silkick", new ModInfo { name = "SilKick", legal = false } },
            { "dtasloi_AdminTag", new ModInfo { name = "DTASLOI Admin (has Console)", legal = false } },
            { "travis", new ModInfo { name = "TravisScott Cosmetics", legal = true } },
            { "forest", new ModInfo { name = "Forest Mod", legal = true } },
            { "cosmetic-set", new ModInfo { name = "Custom Cosmetic Set", legal = true } },
            { "isusing", new ModInfo { name = "Unknown Cheat", legal = false } },
        };
    }
}
