...
namespace ColtixPad.Patches
{
    ...
    public class CosmeticsLoadedPatch
    {
        private static void Postfix(VRRig __instance, string cosmetics)
        {
            ...
            foreach (DictionaryEntry dictionaryEntry in __instance.Creator.GetPlayerRef().CustomProperties)
            ...
            if (legalMods.Count > 0 || illegalMods.Count > 0)
                Notifications.SendNotification($"<color={(illegalMods.Count > 0 ? "red" : "green")}>{(illegalMods.Count > 0 ? "Cheater" : "Modder")}</color> {__instance.playerNameVisible} has <color=green>{(legalMods.Count > 0 ? $"{legalMods.Count} mod{(legalMods.Count > 1 ? "s" : "")}" : "")}</color>{(legalMods.Count > 0 && illegalMods.Count > 0 ? " and " : "")}<color=red>{(illegalMods.Count > 0 ? $"{illegalMods.Count} cheat{(illegalMods.Count > 1 ? "s" : "")}" : "")}</color>");
        }
    }
}